...
async function startServer() {
  ...
  const runScraper = async () => {
    try {
      ...
      if (members.length === 0) {
        $('dt[data-id], dt[data-event]').each((i, el) => {
          const name = $(el).find('.title, h3, h4, span[title]').text().trim() || $(el).find('span').first().text().trim();
          if (name) {
            const category = $(el).find('.description, .category, .industry').text().trim() || 'Corporate Member';
            const imgEl = $(el).find('img');
            let imageStr = imgEl.attr('src');
            // Check if glueup image starts with /
            ...
            const overlayUrl = $(el).attr('data-event') || $(el).find('[data-event]').attr('data-event');
      ...
      console.log(`[Automation] Found ${members.length} members.`);
      if (members.length > 0) {
        saveMembers(members);
